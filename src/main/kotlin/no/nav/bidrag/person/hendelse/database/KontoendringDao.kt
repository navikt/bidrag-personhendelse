...
interface KontoendringDao : JpaRepository<Kontoendring, Long> {
    ...
    @Query(
        "from Kontoendring ke " +
            " where ke.status = no.nav.bidrag.person.hendelse.database.Status.MOTTATT and ke.mottatt < :mottattFør" +
            " and (ke.aktor.publisert is null or ke.aktor.publisert < :publisertFør) "
    )
    fun henteKontoendringerForPublisering(mottattFør: LocalDateTime, publisertFør: LocalDateTime): List<Kontoendring>
